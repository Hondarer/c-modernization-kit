...
`docs/functional-spec/` は、利用側から見た message_catalog の要件と振る舞いを説明する正本です。  
...
- 分類値 (`message_catalog_entry::category`) をライブラリで解釈しないでください。意味と有効な範囲は利用者が決めます。範囲の切り詰めはコマンドの `trace_level_of()` が行います。
...
サンプル コマンドは、カタログを注入した直後に `message_catalog_verify()` を呼び出します。

```bash
./prod/cbin/message-catalog-sample
cd test/src/cmd/messageCatalogDefinitionTest && make test
```
