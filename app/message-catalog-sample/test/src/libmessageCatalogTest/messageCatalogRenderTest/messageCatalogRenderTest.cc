...
class messageCatalogRenderTest : public Test
{
  protected:
    /** 組み立てたメッセージの格納先です。 */
    char dest[64];
    ...
    {
        values[index].kind = MESSAGE_CATALOG_ARGUMENT_KIND_INT32;
    ...
    {
        values[index].kind = MESSAGE_CATALOG_ARGUMENT_KIND_STRING;
        values[index].value.string_value = value;
    }
};

// 位置指定を持たない書式が、そのまま書き込まれることの確認
TEST_F(messageCatalogRenderTest, plain_text)
{
    // Arrange
    int actual_ret;
...
// 位置指定が値へ置き換わることの確認
TEST_F(messageCatalogRenderTest, single_placeholder)
{
    // Arrange
    int actual_ret;
    set_int32(0, 42);
...
TEST_F(messageCatalogRenderTest, reordered_placeholder)
{
    // Arrange
    int actual_ret;
...
TEST_F(messageCatalogRenderTest, repeated_placeholder)
{
    // Arrange
    int actual_ret;
    set_int32(0, 7);
...
TEST_F(messageCatalogRenderTest, escaped_braces)
{
    // Arrange
    int actual_ret;
    set_int32(0, 123);
...
// 引数種別ごとの文字列表現の確認
TEST_F(messageCatalogRenderTest, argument_kind_text)
{
    // Arrange
    ...
    // Act / Assert
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_STRING;
    ...
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_INT64;
    ...
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_UINT64;
    ...
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_HEX64;
    ...
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_SIZE;
    ...
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_DOUBLE;
    ...
    values[0].kind = MESSAGE_CATALOG_ARGUMENT_KIND_ERROR_CODE;
...
TEST_F(messageCatalogRenderTest, unknown_argument_kind)
{
    // Arrange
    int actual_ret;
...
TEST_F(messageCatalogRenderTest, truncated_output)
{
    // Arrange
    char small_dest[6];
    int actual_ret;
...
TEST_F(messageCatalogRenderTest, truncated_after_full)
{
    // Arrange
    char small_dest[4];
    int actual_ret;
...
TEST_F(messageCatalogRenderTest, exact_fit)
{
    // Arrange
    char small_dest[4];
    int actual_ret;
...
TEST_F(messageCatalogRenderTest, invalid_format)
{
    // Arrange
    int actual_ret;
    set_int32(0, 1);
