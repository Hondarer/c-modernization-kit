...
`unicast_bidir` / `unicast_bidir_n1` では、各エンドポイントまたは各ピアが独立して PING 送信・応答を行います（後述「双方向 UDP のヘルスチェック動作」参照）。
...
TCP でも `tcp_health_interval_ms` 周期で PING を送信します。
PING 送信自体は UDP と同様にデータ送信頻度の影響を受けません。
...
TCP は PING をトラフィック量に関わらず必ず周期送信するため、両端が互いの生死を独立して監視できます。
