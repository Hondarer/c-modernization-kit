...
| 値 | 定数 | 意味 |
|---|---|---|
| `0` | `POTR_PING_STATE_UNDEFINED` | 不定 (片方向 / まだ PING 未受信) |
| `1` | `POTR_PING_STATE_NORMAL` | 正常 (定周期で PING 受信中) |
| `2` | `POTR_PING_STATE_ABNORMAL` | 異常 (PING 途絶) |

片方向通信 (type 1-6) はそもそも相手から PING を受信しないため、全パスを `UNDEFINED` で送出します。双方向通信 (type 7-10) は実際の受信状態を格納します。
