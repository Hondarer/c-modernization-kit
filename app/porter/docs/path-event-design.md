...
POTR_EVENT_CONNECTED, POTR_EVENT_DISCONNECTED の直前に、POTR_EVENT_PATH_CONNECTED, POTR_EVENT_PATH_DISCONNECTED がコールされる。この2つのイベントは相互に強く関連しているため、コールバックは必ず連続して行われる。
