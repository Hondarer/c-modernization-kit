...
`*_health_interval_ms > 0` のとき、UDP/TCP いずれもデータ送信の有無に関係なく設定周期で PING を送信します。
...
| 設定 | 効果 |
|---|---|
| `udp_health_interval_ms = 0` | UDP 通信種別で周期 PING を送信しない |
| `udp_health_timeout_ms = 0` | UDP 通信種別でタイムアウト監視を行わない |
| `tcp_health_interval_ms = 0` | TCP 通信種別で周期 PING を送信しない |
| `tcp_health_timeout_ms = 0` | TCP 通信種別で PING 要求 / 応答のタイムアウト監視を行わない |
