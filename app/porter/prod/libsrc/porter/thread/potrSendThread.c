...
static void flush_packed(struct PotrContext_ *ctx, size_t packed_len)
{
        ...
        if (ctx->tcp_active_paths > 0)
        {
            ...
            for (i = 0; i < ctx->n_path; i++)
            {
                ...
                if (pr > 0)
                {
                    ...
                    POTR_MUTEX_LOCK(&ctx->tcp_send_mutex[i]);
                    potr_tcp_send(ctx->tcp_conn_fd[i], ctx->send_wire_buf, wire_len);
                    POTR_MUTEX_UNLOCK(&ctx->tcp_send_mutex[i]);
                }
                else
                {
                    if (ctx->buf_full_suppress_cnt[i] == 0)
    ...
    else
    {
        ...
        {
            ctx->last_valid_data_send_ms = potr_get_monotonic_ms();
...
/* 送信スレッド本体 */
POTR_THREAD_FUNC(send_thread_func)
{
        ...
        /* パッキング試行 */
        {
            ...
            if (!(first.flags & POTR_FLAG_MORE_FRAG))
            {
                ...
                if (pack_wait_ms > 0)
                {
                    ...
                    uint64_t      deadline = potr_get_monotonic_ms() + pack_wait_ms;
                    PotrPayloadElem next;

                    for (;;)
                    {
                        uint64_t now = potr_get_monotonic_ms();
                        uint32_t remaining;
                        size_t   elem_size;
...
int potr_send_thread_start(struct PotrContext_ *ctx)
{
    ...
    POTR_MUTEX_INIT(&ctx->send_window_mutex);
    if (potr_thread_create(&ctx->send_thread, send_thread_func, ctx) != 0)
    {
        ctx->send_thread_running = 0;
        POTR_MUTEX_DESTROY(&ctx->send_window_mutex);
        return POTR_ERROR;
    }
...
void potr_send_thread_stop(struct PotrContext_ *ctx)
{
    ...
    potr_thread_join(&ctx->send_thread);
    POTR_MUTEX_DESTROY(&ctx->send_window_mutex);
}
