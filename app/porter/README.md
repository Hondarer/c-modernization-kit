...
| ヘルスチェック | 片方向 type 1-6 は「最後の PING または有効 DATA 送信」から `health_interval_ms` 経過時だけ PING を送り、有効な PING / DATA 受信で疎通を維持する。双方向系は従来どおり定周期 PING を基準に切断検知し、各パスの PING 受信状態変化時に割り込み PING も送信 |
| マルチパス | 最大 4 経路の並列送信 |
