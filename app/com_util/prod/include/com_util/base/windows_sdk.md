...
`windows_sdk.h` は、`com_util` の公開ヘッダーから Windows SDK を参照する際の共通入口です。
...
`com_util` の公開ヘッダーで Windows SDK の型や API が必要な場合は、`<windows.h>` を直接取り込まず、`<com_util/base/windows_sdk.h>` を参照します。
...
そのため、`com_util` 側で共通ルールを持つ必要があります。
