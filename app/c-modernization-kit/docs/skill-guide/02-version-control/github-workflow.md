...
このリポジトリは GitHub でホストされており、GitHub Actions による CI/CD パイプラインを使用しています。PR のマージ時に自動ビルド・テスト・ドキュメント公開が実行されます。チームでこのリポジトリを活用するには、GitHub Flow の基本的なワークフローを理解することが重要です。
...
# 2. 変更をコミット
git add app/calc/test/src/libcalcbaseTest/addTest/addTest.cpp
...
説明:
## 変更内容
- app/calc/test/src/libcalcbaseTest/divideTest/ にゼロ除算のテストケースを追加
...
- [GitHub Actions (スキル ガイド)](../06-ci-cd/github-actions.md) - GitHub Actions の学習マテリアル
