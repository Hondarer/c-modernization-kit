...
Markdown は、プレーンテキストで構造化されたドキュメントを書くための軽量マークアップ言語です。`#` で見出し、`-` で箇条書き、`` ` `` でインライン コードなどを表現します。GitHub・GitLab・各種 Wiki で一般的に使われており、HTML に変換して表示されます。

このリポジトリのドキュメント (`docs/*.md`・このスキル ガイド) はすべて Markdown で記述されています。Doxygen によって生成されるドキュメントも Doxybook2 を経由して Markdown 形式になり、最終的に Pandoc で HTML・docx に変換されます。また、GitHub 上でのリポジトリの説明 (README.md)、PR の説明、Issues のコメントもすべて Markdown で書きます。
