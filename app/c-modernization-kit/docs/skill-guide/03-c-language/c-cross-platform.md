...
このリポジトリは Linux (GCC) と Windows (MSVC) の両方をサポートするクロスプラットフォーム設計になっています。`app/calc/prod/include/libcalc.h` では、Windows の DLL エクスポート宣言 (`__declspec(dllexport)`) と Linux の可視性属性 (`__attribute__((visibility("default")))`) を条件コンパイルで切り替えています。同様のパターンは `app/*/prod/` 配下の他のヘッダー ファイルにも見られます。
