...
公開するカタログでは、カタログ項目へ `value` を記述します。  
列挙値は利用側のバイナリへ埋め込まれるため、定義の並べ替えで値が変化すると事前の通知なしに非互換の変更となります。

廃止する項目は定義から削除し、値を欠番として保持してください。  
値の再利用はしないでください。
...
コンテキスト引数を追加する場合は、`catalog_settings.jsonc` の `arguments` へ `index` を添えて記述してください。  
公開するカタログでは `index` が必須です。記述順を変更しても位置指定が変化しないようにするためです。  
