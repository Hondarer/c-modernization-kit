 ...
 *  @version        0.1.0
 *
 *  書き込み側は、フィルター オブジェクトを共有メモリへ公開するだけで、読み取り側のフィルター スロットには何もしません。\n
 *  読み取り側は、トレース出力のたびに世代番号をロックなしで読み (緩いチェック)、変化していれば
 *  ロックを取って読み直し (最終チェック)、フィルター オブジェクトを複製してフィルター スロットへ適用します。
 *
...
extern "C"
{
     ...
     *
     *  読み取り側のフィルター スロットには何もしません。読み取り側は次の取り込みで変化に気付きます。
     *
     *  @par            スレッド セーフ
     *  本関数はスレッド セーフです。\n
     ...
     *  @return         ロックを取れない場合は、その結果コードを返します。
     *
     *  通常は、世代番号をロックなしで 1 回読み、取り込み済みの世代と一致すれば直ちに戻ります。\n
     *  一致しなければロックを取って世代を読み直し、変化していれば複製してスロットへ適用します。\n
     *  適用に失敗した場合も取り込み済みの世代を更新し、同じ世代の取り込みを繰り返しません。
