...
| `list` | このプロセスが取り込み済みの条件式を、カタログのメタ情報を用いた説明文とともに表示します |
...
同じパスを指定して複数のコマンドを起動すると、どのコマンドの `apply` も、すべてのコマンドのトレース出力へ配布されます。

`apply` は共有メモリへ公開するだけで、自分のプロセスの条件も、その場では変わりません。  
各プロセスは、次のトレース出力の時点で公開内容の変化 (世代の変化) に気付いて取り込みます。  
