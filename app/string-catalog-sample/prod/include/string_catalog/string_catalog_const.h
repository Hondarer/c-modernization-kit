 ...
 *
 *  書式中の位置指定は `{0}` から `{31}` までとなります。添字は 0 相対です。\n
 *  書式解析は 2 桁までの添字を受け付けるため、この値の上限は 100 です。\n
 *  100 を超える値を設定する場合は、書式解析の桁数の上限も合わせて広げてください。
 *
 *  この値は 1 件分のカタログと、組み立て 1 回あたりのスタックの大きさを決めます。\n
 *  @ref string_catalog_entry は引数の種別を配列で持ち、組み立ては値の配列をスタックへ置きます。\n
 *  どちらも実際の引数の個数ではなく、この値に比例します。
 */
