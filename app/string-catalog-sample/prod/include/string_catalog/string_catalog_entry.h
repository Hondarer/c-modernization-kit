 ...
 *  配列とインデックス表を @ref string_catalog へまとめ、組み立て API の呼び出しごとに渡します。\n
...
extern "C"
{
     ...
     *
     *  カタログの配列と、文字列 ID から配列のインデックスを参照する表を 1 つにまとめた値です。\n
     ...
     *
     *  @ref string_catalog::id_index は、文字列 ID からカタログ項目を検索する探索コストを低減するための変換表です。\n
     *  文字列 ID をインデックスとして @ref string_catalog::entries のインデックスを格納し、
    ...
    typedef struct string_catalog
    {
        ...
        const int *id_index; /**< 文字列 ID をインデックスとして entries のインデックスを参照する変換表です。不要な場合は NULL です。 */
