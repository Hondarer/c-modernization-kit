...
ifndef MAKEFW_SYNC_EVAL
    ...
    _SAMPLE_TRACE_GEN_STATUS := $(shell python3 "$(APP_DIR)/c-platform/bin/string_catalog_gen.py" \
        "$(MYAPP_DIR)/prod/src/cmd/string-catalog-sample/sample_trace.jsonc" \
        --out-dir "$(MYAPP_DIR)/prod/src/cmd/string-catalog-sample/gen" --if-newer >&2; echo $$?)
    ifneq ($(_SAMPLE_TRACE_GEN_STATUS),0)
        $(error カタログ定義からの生成に失敗しました。上記のメッセージを確認してください)
    endif
endif
