...
`key` は処理から項目を参照する文字列キーで、列挙定数の名前になります。列挙値は、カタログ定義の `value` です。`value` を省略した場合は、定義の並び順に基づいて 1 始まりの値を生成器が割り当てます。`value` は、カタログ内のすべての文字列に記述するか、すべてで省略してください。  
...
位置指定は `index` によって決定されます。  
記述順の変更によって番号が変化すると書式が参照する値が変わるため、公開するカタログでは記述を必須としています。  
カタログ項目の `value` と同様の考え方です。
