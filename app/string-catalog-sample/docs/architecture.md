...
- カタログはライブラリ側では保持せず、利用側が用意したカタログを呼び出しごとに渡します。
...
カタログはライブラリ側では保持しません。  
利用側が定義し、`string_catalog` へまとめて、API の呼び出しごとに渡します。

`string_catalog` は、カタログ配列と添字テーブルを 1 つにまとめたカタログ識別オブジェクトです。  
すべてのメンバーを初期化子で設定できるため、`const` の静的記憶域期間を持つ値として定義できます。  
初期化関数を持たないため、初期化順序を考慮する必要がありません。

ライブラリはこの値を保持しません。  
そのため、1 つのプロセスで複数のカタログを同時に扱えます。  
ライブラリ本体のカタログと、それを利用するアプリケーションのカタログを、別々に管理できます。
...
| `sample_trace_level.h` | 分類値の列挙と、その意味付け | 手動作成 |
...
分類値の列挙 `sample_trace_level` は別ヘッダーで手動定義します。  
...
カタログは利用側で静的初期化されるため、ライブラリは関数の入口で値の妥当性を確認します。  
...
この簡易関数をライブラリではなく生成物へ配置するのは、既定カタログという状態をライブラリに持たせないためです。  
...
カタログ指定を省略する簡易関数 (`sample_messages_format()` など) はカタログ固有の関数であり、モジュール接頭辞のままです。
