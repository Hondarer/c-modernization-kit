...
`samplecatalog_trace.jsonc` は、カタログ定義の `text_prefix` で全書式へ共通の前置を指定しています。
...
前置と後置は、カタログ定義ごとに言語別で指定します。  
生成器が生成の時点で `texts` へ結合するため、ライブラリは結合後の書式だけを受け取ります。

結合では区切りを挿入せず、単純に連結します。  
そのため、番号と本文を分ける空白は前置の文字列へ含めています。

指定のない言語は、書式と同じくニュートラル言語へフォールバックします。  
この app では前置を言語で変える必要がないため、`neutral` だけを記述しています。

結合するのは `texts` だけであり、`notes` は定義のままです。  
`notes` は組み立てた文字列ではなく、定義を読む人への補足であるためです。

位置指定は構文だけを検査し、添字が値を受け取るかどうかは検査しません。  
共通の前置と後置は、引数定義の異なる複数の書式へ結合されるためです。

この指定により、連番の `{46}` を各書式へ個別に書く必要がなくなります。  
書式を追加した場合も、前置の記述漏れによって連番が欠ける状態が起こりません。
