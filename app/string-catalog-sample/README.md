...
出力言語はプロセス単位で 1 つ設定し、文字列を組み立てるたびに指定する必要はありません。  
言語を設定していないプロセスは、実行環境が示す表示言語に対応する言語を使用します。  
対応する言語が存在しない場合は、ニュートラル言語を使用します。

カタログは cplat 側では保持しません。  
利用側で文字列キーの列挙型とカタログ配列を定義し、カタログ識別オブジェクトに集約して呼び出しごとに渡します。  
この app では、カタログ定義から生成器が列挙型とカタログ配列を出力します。

カタログの保持形態には 2 種類あり、この app はその双方を例示しています。  
コマンドがカタログを同梱する形態と、ライブラリがカタログを外部公開し、別のコマンドがそれを参照する形態です。

| 実行ファイル | カタログの配置形態 | 利用側における参照形式 |
|---|---|---|
...
[ライブラリのカタログから組み立てた文字列]
  ...
  要求されたサイズ 4096 は上限 1024 を超えています。
```
