...
- ソリューション エクスプローラーに構成選択 UI がありません。
...
1. **統合ターミナルを使用** (推奨)
    ...
    - `Directory.Build.props` の設定により、デフォルトで RelWithDebInfo が使用されます。
...
- **パフォーマンス劣化の早期発見**: 開発中も常に最適化されたコードでテスト可能
- **本番環境に近い状態でのテスト**: より実環境に近い条件でのテスト実行
- **問題発生時のデバッグが容易**: デバッグ シンボルにより、詳細なスタック トレースを取得可能
