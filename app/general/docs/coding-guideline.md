...
| 値の意味 | 選択する型 | 使用条件 |
|---|---|---|
| オブジェクト ポインターとの往復変換、アドレス値 | `uintptr_t` | ポインター幅へ追従させます。 |
| 負の無効値を持つ OS ネイティブ ハンドル | `intptr_t` | OS API がポインター幅の整数と負のセンチネル値を要求する場合に限定します。 |
| オブジェクトのバイト サイズ、要素数、`sizeof` の結果、それらと同じ範囲の配列インデックス | `size_t` | オブジェクトに関する、負値を取らない値に使用します。 |
| 同一配列内の 2 つのポインターの差 | `ptrdiff_t` | 異なる配列を指すポインター同士の減算には使用しません。 |
| POSIX I/O API が返す処理済みバイト数とエラー値 | `ssize_t` | Linux 実装内の POSIX API 境界に限定し、Windows 側へ露出させません。 |
| POSIX のファイル位置、ファイル サイズ、ファイル オフセット | `off_t` | Linux 実装内の POSIX API 境界に限定し、Windows 側へ露出させません。 |
| C / POSIX 時刻 API が扱う時刻の秒部 | `time_t` | `time()` の戻り値や `struct timespec::tv_sec` などに使用し、任意の期間には使用しません。 |
| クロスプラットフォームで受け渡す絶対時刻、単調時刻 | 各 app が定める標準時刻型 | 標準時刻型の定めがある場合はそれに従います (各 app の特化事項を参照)。 |
...
> その値を長さやインデックスに使うと、バッファー外アクセスにつながります。
...
- 長さ、インデックス、残量など、回り込みが意味を持たない値では、演算の前に範囲を検査します
...
- 単項演算子とメンバー参照、インデックス参照、関数呼び出しの組み合わせ (`*p`、`p->member`、`arr[i]`、`f(x)`)
...
- 複数の `.c` ファイルで共有する短い補助処理であり、通常関数にすると目的に対して呼び出しの負担が大きい場合
- 既存の関数へ薄く転送するだけの関数を多数用意する場合。実体を持つ翻訳単位と公開シンボルを増やさずに済みます

後者は、型付きの入口を機械的に並べる場合に当てはまります。  
転送先の契約を変えず、本体が 1 つの呼び出しに収まることを条件とします。  
