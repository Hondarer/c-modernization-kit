...
- 単体テストは小さな単位を素早く検証できるため、失敗を早期に検出できます。
- 大規模な UI テストより安価に自動で実行できるため、結果的に全体の工数を抑えやすくなります。
...
別管理でも不可能ではありませんが、更新の不整合が生じやすくなります。
...
まずは「プッシュでテストとドキュメント生成が実行される」最小構成から始め、必要に応じて段階的に拡張することを推奨します。
