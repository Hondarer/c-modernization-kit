...
通常の利用者は `util/trace/trace.h` を経由して使い、syslog 固有 API が必要な場合だけ `util/trace/trace_syslog.h` を直接扱います。
...
`util/trace/trace_syslog.h` は、syslog sink を直接生成したい場合の lower layer です。  
