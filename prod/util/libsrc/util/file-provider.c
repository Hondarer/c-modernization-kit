...
static char level_char(int level)
{
    switch (level)
    {
    case TRACE_FILE_LV_CRITICAL: return 'C';
    case TRACE_FILE_LV_ERROR:    return 'E';
    case TRACE_FILE_LV_WARNING:  return 'W';
    case TRACE_FILE_LV_INFO:     return 'I';
    default:                     return 'V';
    }
}
    ...
    trace_file_provider_init(const char *path, size_t max_bytes, int generations)
{
    ...
    handle = (trace_file_provider_t *)malloc(sizeof(trace_file_provider_t));
    if (handle == NULL)
    {
        return NULL;
    }
    ...
    handle->max_bytes    = (max_bytes > 0)    ? max_bytes    : TRACE_FILE_DEFAULT_MAX_BYTES;
    handle->generations  = (generations > 0)  ? generations  : TRACE_FILE_DEFAULT_GENERATIONS;
