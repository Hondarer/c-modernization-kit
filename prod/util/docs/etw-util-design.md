...
```c
/**
 *  プラットフォーム非依存のプロバイダ参照型。
 *  Windows: TraceLoggingHProvider (TraceLoggingProvider.h が必要)
 *  Linux:   void * (スタブ用)
 */
#ifdef _WIN32
    #include <TraceLoggingProvider.h>
    typedef TraceLoggingHProvider etw_provider_ref_t;
#else
    typedef void *etw_provider_ref_t;
#endif
 ...
 *
 *  Windows: TRACELOGGING_DEFINE_PROVIDER(var, name, guid) に展開する。
 *  Linux:   static void *var = NULL; に展開する (スタブ用)。
 *
...
#else
    #define ETW_UTIL_DEFINE_PROVIDER(var, name, guid) \
        static etw_provider_ref_t var = NULL
#endif
...
```c
etw_provider_t *etw_provider_init(etw_provider_ref_t provider_ref)
    { (void)provider_ref; return NULL; }
int etw_provider_write(etw_provider_t *handle, int level, const char *message)
    { (void)handle; (void)level; (void)message; return 0; }
void etw_provider_fini(etw_provider_t *handle)
    { (void)handle; }
```
...
2. `potrLogConfig()` 内:
   ...
   - `POTR_LOG_OFF` 設定時に `etw_provider_fini()` を呼ぶ
   - 多重登録対策: `etw_provider_init()` 前に既存ハンドルを `etw_provider_fini()` する
...
| `potrLogConfig` の多重呼び出し | `etw_provider_init()` 前に既存ハンドルを `etw_provider_fini()` してから再初期化 |
| 終了時の fini 忘れ | `POTR_LOG_OFF` 設定時に `etw_provider_fini()`。プロセス終了時未解放は OS 回収で問題なし |
...
| `etw_provider_init(...)` | — (スタブが NULL を返す) |
| `etw_provider_fini(...)` | — (スタブが何もしない) |
