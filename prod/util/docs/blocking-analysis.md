# trace-util 永久ブロッキングリスク調査

## 調査背景

`trace_dispose` のスレッドセーフ化として、`trace_stop` → `trace_dispose` の呼び出し順序を
内部で保証する設計を採用した（`trace-provider.c` の `config_rwlock` 参照）。

この設計では `trace_dispose` が排他ロックを取得するまで待機するため、
...
### 1. `config_lock_shared` (trace-provider.c)

**対象コード**: `trace_write` / `trace_writef` / `trace_hex_write` / `trace_hex_writef`

**ブロッキング条件**:
`trace_dispose` または `trace_stop` が排他ロックを保持している間、新たな共有ロック取得はブロックする。
...
### 2. ファイルプロバイダ mutex (file-provider.c)
...
### 3. `write()` with `O_DSYNC` (file-provider.c) ⚠️ 既知制限
...
### 4. `syslog()` (syslog-provider.c) ⚠️ 既知制限
...
SOCK_STREAM 接続が懸念される場合は OS トレース (`os_level`) を `TRACE_LV_NONE` に
設定してファイルトレースのみを使用することを検討すること。

---

### 5. ETW (etw-provider.c, Windows のみ)
...
### trace-provider.c: `config_lock_shared_timed`
...
2. **旧来の SOCK_STREAM syslog 環境**: `syslog()` のブロックは回避手段がない。
   懸念がある場合は `trace_modify_ostrc(handle, TRACE_LV_NONE)` で OS トレースを無効化すること。

3. **`trace_stop` / `trace_dispose` のブロック時間**: 上記のブロッキングが発生した場合、
   `trace_stop` が排他ロック取得のために最大 100 ms 待機する可能性がある。
