...
**採用方針**: `trace-etw-util` は **TraceLogging**（ヘッダーオンリー）を採用する。
...
**ETW ヘルパー関数群 (`trace-etw-util`)** を用意する。
...
| ファイル | 種別 | 役割 |
|---------|------|------|
| `prod/util/include/trace-etw-util.h` | 公開ヘッダー | API 宣言・不透明ハンドル型（TraceLoggingベース） |
| `prod/util/libsrc/util/etw-provider.c` | 実装 | Windows: TraceLogging 実装 (Windows 専用) |
| `prod/util/libsrc/util/makepart.mk` | ビルド設定 | `ETW_UTIL_EXPORTS` 追加 |

### 公開 API 設計 (`trace-etw-util.h`) (TraceLogging 前提)
...
### 実装方針 (`etw-provider.c`) (TraceLogging)
...
呼び出し元は `#ifdef _WIN32` の中でのみ trace-etw-util を使用すること。
...
ifeq ($(OS),Windows_NT)
    ...
    CFLAGS   += /DFMTIO_UTIL_EXPORTS /DETW_UTIL_EXPORTS
    CXXFLAGS += /DFMTIO_UTIL_EXPORTS /DETW_UTIL_EXPORTS
...
`trace-etw-util` が完成したら `potrLog.c` を改修する。

### 変更内容

1. `#ifdef _WIN32` ブロックに `#include <trace-etw-util.h>` と静的ハンドルを追加:
...
|------|---------|
| GUID 生成 (`uuidgen`) | 5 分 |
| `trace-etw-util.h` ヘッダー設計・作成 | 30〜60 分 |
| `etw-provider.c` Windows 実装 (TraceLogging) | 1〜2 時間 |
...
| フェーズ | 内容 | 実現性 |
|---------|------|--------|
| 1 | `prod/util/trace-etw-util` ヘルパーライブラリ新設 | ✅ 高い |
| 2 | `potrLog.c` の `trace-etw-util` 利用への書き換え | ✅ 高い |
