...
prod/simplecomm/
├── sample-config/
│   └── simplecomm-services.conf    # サービス定義ファイル (サンプル)
├── makefile
├── include/
│   ├── libsimplecomm_const.h       # 定数定義
│   ├── libsimplecomm_type.h       # 型定義
│   └── libsimplecomm.h             # 公開 API
├── libsrc/
│   ├── simplecommcore/             # 静的ライブラリ (プロトコルロジック)
...
│   └── simplecomm/                 # 動的ライブラリ (UDP ソケット層)
│       ├── commContext.h           # CommHandle 実体 (内部非公開)
│       ├── commOpenService.c       # サービス開設
│       ├── commSend.c              # データ送信
│       ├── commClose.c             # サービス終了
│       └── commRecvThread.c        # 受信スレッド (内部)
└── src/
...
typedef struct {
    ...
    uint16_t flags;                     // COMM_FLAG_DATA / ACK / NACK (NBO)
...
スライディングウィンドウ方式。送信側・受信側それぞれ `CommWindow` 構造体で管理する。
...
`commContext.h` でプラットフォーム差異を吸収する。
...
send sample-config/simplecomm-services.conf 2001 "Hello, World!"
```
...
recv sample-config/simplecomm-services.conf 2001
```
