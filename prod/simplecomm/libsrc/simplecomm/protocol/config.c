 ...
 *******************************************************************************
 */
int config_load_global(const char *config_path, CommGlobalConfig *global)
{
    FILE *fp;
    ...
    /* デフォルト値で初期化 */
    global->window_size           = (uint16_t)COMM_DEFAULT_WINDOW_SIZE;
    global->max_payload           = (uint16_t)COMM_DEFAULT_MAX_PAYLOAD;
...
static void apply_service_kv(const char *key, const char *val,
                              CommServiceDef *current)
{
    if (strcmp(key, "type") == 0)
    {
        if (strcmp(val, "unicast") == 0)
        {
            current->type = COMM_TYPE_UNICAST;
        }
        else if (strcmp(val, "multicast") == 0)
        {
            current->type = COMM_TYPE_MULTICAST;
        }
        else if (strcmp(val, "broadcast") == 0)
        {
            current->type = COMM_TYPE_BROADCAST;
        }
    }
 ...
 *  @return         成功時は COMM_SUCCESS、サービスが見つからない場合は COMM_ERROR を返します。
 *
 *  @details
                        ...
                        CommServiceDef *def)
{
        ...
        if (trimmed[0] == '[')
        {
            ...
            if (close != NULL)
            {
                    ...
                    atoi(section + 8) == service_id)
                {
                    ...
                    def->ttl        = (uint8_t)COMM_DEFAULT_TTL;
    ...
    fclose(fp);
    return found ? COMM_SUCCESS : COMM_ERROR;
}
