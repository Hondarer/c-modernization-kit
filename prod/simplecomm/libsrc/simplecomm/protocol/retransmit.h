...
typedef struct
{
    RetransmitEntry entries[COMM_MAX_WINDOW_SIZE]; /**< 再送管理エントリ配列。 */
...
extern "C"
{
    ...
    extern void retransmit_init(CommRetransmit *rt,
                                uint32_t timeout_ms, uint8_t max_retry);
    extern int  retransmit_register(CommRetransmit *rt,
                                    uint32_t seq_num, uint32_t now_ms);
    extern int  retransmit_ack(CommRetransmit *rt, uint32_t seq_num);
    extern int  retransmit_check(CommRetransmit *rt, uint32_t now_ms,
