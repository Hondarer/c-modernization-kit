                  ...
                  size_t         src_len)
{
    ...
    if (*dst_len < COMM_COMPRESS_HEADER_SIZE + 1U)
    {
        return -1;
    }
    ...
    memcpy(dst, &orig_len_nbo, COMM_COMPRESS_HEADER_SIZE);

    /* raw DEFLATE (windowBits = -15) で圧縮 */
    memset(&z, 0, sizeof(z));
    z.next_in   = (Bytef *)(uintptr_t)src;
    z.avail_in  = (uInt)src_len;
    z.next_out  = dst + COMM_COMPRESS_HEADER_SIZE;
    z.avail_out = (uInt)(*dst_len - COMM_COMPRESS_HEADER_SIZE);
    ...
    *dst_len = COMM_COMPRESS_HEADER_SIZE + (size_t)z.total_out;
    return 0;
}

/* doxygen コメントはヘッダに記載 */
int comm_decompress(uint8_t       *dst,
                    size_t        *dst_len,
                    const uint8_t *src,
                    size_t         src_len)
{
    ...
    if (dst == NULL || dst_len == NULL || src == NULL
        || src_len <= COMM_COMPRESS_HEADER_SIZE)
    {
        return -1;
    }

    /* 先頭 4 バイトから元サイズを取得 */
    memcpy(&orig_len_nbo, src, COMM_COMPRESS_HEADER_SIZE);
    ...
    /* raw DEFLATE を解凍 */
    memset(&z, 0, sizeof(z));
    z.next_in   = (Bytef *)(uintptr_t)(src + COMM_COMPRESS_HEADER_SIZE);
    z.avail_in  = (uInt)(src_len - COMM_COMPRESS_HEADER_SIZE);
