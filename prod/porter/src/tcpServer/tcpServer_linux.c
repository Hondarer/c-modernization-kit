...
/**
 *******************************************************************************
 *  @brief          プラットフォーム初期化 (Linux: g_session_fn の設定のみ)。
...
/**
 *******************************************************************************
 *  @brief          内部起動引数を処理します (Linux: 常に 0 を返します)。
 *  @param[in]      argc コマンドライン引数の数 (未使用)。
 *  @param[in]      argv コマンドライン引数の配列 (未使用)。
 *  @return         常に 0 を返します。Linux には内部起動モードがありません。
 *******************************************************************************
 */
 ...
 *  @param[in]      port 待ち受けポート番号。
 *
 *  SIGCHLD ハンドラを設定後、listen ソケットを作成して accept ループを開始します。
 *  接続を受け付けるたびに fork() で子プロセスを生成し、g_session_fn() に委譲します。
 *******************************************************************************
 */
 ...
 *  ワーカープロセスを fork します。SIGINT / SIGTERM を受信するまで待機し、
 *  受信後は全ワーカーに SIGTERM を送信して終了を待ちます。
 *******************************************************************************
 */
