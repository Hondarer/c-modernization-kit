...
- **受信コールバックは全通信種別で共通インターフェース**とする (`peer_id` 引数付き)
  - 1:1 動作および他の通信種別では `peer_id` は常に 0
...
> **注意**: `src_port` のみ指定 (`src_addr` なし) は不正な設定としてエラーとする。
...
1:1 モードではピアテーブルは使用せず、既存の PotrContext 内の単一セッション情報で管理する (`peer_id` は常に 0)。
...
#define POTR_PEER_ALL      UINT32_MAX  /* 全ピア宛 (N:1 一斉送信用) */
```

- N:1 モードでは、有効なピア ID は常に 0 以外の値となる (ピア ID 生成ロジックにより保証)
- 1:1 モードおよびその他の通信種別では `peer_id` には常に **0** を使用する
  - 0 は「特定ピアを指定しない (= 唯一のピアまたは N/A)」を意味する
 ...
 * 1:1 モードおよびその他の通信種別では peer_id は常に 0。
 *
...
| `potrSend(handle, peer_id, ...)` | `peer_id` で送信先ルーティング。0 は POTR_ERROR |
...
| 関数 | 1:1 モード時の動作 |
|------|------------------|
| `potrSend(handle, 0, ...)` | 既存動作と同等 (唯一のピアへ送信) |
...
int main(void)
{
    ...
    {
        potrSend(handle, 0, "hello", 5, 0);  /* peer_id=0 で既存動作 */
        sleep(5);
...
- 予約値: `0` (特定ピアを指定しない) および `UINT32_MAX` (`POTR_PEER_ALL`)
...
```c
typedef struct {
    PotrPeerId  peer_id;    /* 送信先ピア (N:1 用。1:1 では 0) */
    /* ... 既存フィールド ... */
} PotrSendQueueEntry;
```

送信スレッドはデキュー時に `peer_id` でピアテーブルを検索し、該当する `PotrPeerContext` の `dest_addr` を解決して sendto する。1:1 モードでは `peer_id = 0` であり、既存の送信先アドレスを使用する。
...
| キー | セクション | 型 | デフォルト | 説明 |
|-----|-----------|-----|----------|------|
| `max_peers` | `[service.N]` | uint32 | 32 | N:1 モード時の最大同時接続ピア数。1:1 モードでは無視される。超過時は `POTR_LOG_ERROR` を記録しパケットを破棄する (セクション 2.4 参照) |
...
| ファイル | 変更内容 |
|---------|---------|
| `include/porter_type.h` | `PotrPeerId` 型定義追加、`PotrRecvCallback` に `peer_id` 引数追加、`POTR_PEER_ALL` 定数追加 |
...
| 既存テスト全般 | `PotrRecvCallback` / `potrSend()` シグネチャ変更に合わせた修正 (`peer_id = 0` の追加) |
...
| 項目 | 状態 | 説明 |
|------|------|------|
| `peer_id = 0` の二重意味 | **整合** | 1:1 モードでは「唯一のピア / N/A」、N:1 モードでは「無効 (有効なピア ID は常に非ゼロ)」。文脈で一意に決まるため矛盾なし |
...
| `src_port` のみ指定 (`src_addr` なし) | **整合** | 設定エラー (POTR_ERROR) として明示的に処理 |
