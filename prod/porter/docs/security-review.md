...
- N:1 モードでは、GCM 認証前に `session triplet` を基に新規ピアを作成するため、未認証の peer slot 枯渇 DoS への余地がある。
...
### 1. N:1 モードでは未認証の peer slot 枯渇 DoS への余地がある
...
- 受信スレッドは `packet_parse()` と `service_id` / `src_addr` チェックの後、N:1 モードではまず `peer_find_by_session()` を行い、未知なら `peer_create()` でピアを確保する。  
  参照: `prod/porter/libsrc/porter/thread/potrRecvThread.c:1759-1798`
- GCM 復号・タグ検証はその後に行われる。  
  参照: `prod/porter/libsrc/porter/thread/potrRecvThread.c:1819-1889`
- `peer_create()` はスロット確保・ウィンドウ初期化・フラグメントバッファ確保を行い、`ctx->n_peers` を増やす。  
  参照: `prod/porter/libsrc/porter/potrPeerTable.c:307-410`
- さらに N:1 モードでは、`src_port` 未指定時に送信元を全許可する。  
  参照: `prod/porter/libsrc/porter/thread/potrRecvThread.c:643-649`

#### 影響

- `max_peers` に達すると正規クライアントの新規接続を拒否する。
- 閉域網で送信元が十分に制限されていれば現実的な発生可能性は下がるが、暗号化を有効にしていても認証前に資源を消費する点は改善候補である。

### 2. GCM ノンス設計が `session_id` の強度に依存しており、nonce reuse の設計リスクがある
...
### 3. 相手認証は事前共有鍵の所持確認に留まり、ピア単位の認証・認可までは提供しない
...
### 4. 鍵管理はシンプルだが、設定ファイルの事前共有鍵と SHA-256 直 hash に依存している
...
### 5. `CLOCK_REALTIME` ベースのセッション採用には、時刻逆行時の運用制約がある
...
1. 暗号認証前に資源を消費する N:1 DoS 経路がある。  
2. GCM ノンス設計が `session_id` 生成の強度に依存している。  
3. 相手認証は共有鍵の所持確認止まりで、ピア固有の認証になっていない。  
4. 鍵管理は事前共有鍵 + 平文設定 + SHA-256 直 hash に依存している。  
5. `CLOCK_REALTIME` ベースのセッション採用に可用性上の既知制限がある。  

従って、porter の暗号化は **閉域網での利用において十分に有用** である一方、**より強い脅威モデルに耐えるためには追加改善の余地がある**、というのが妥当な評価です。

## 推奨対応

- N:1 モードでは、新規 peer 作成前に GCM 認証を通すか、未認証ピア用の厳しいレート制限・クォータを設ける。
