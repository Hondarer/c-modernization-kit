...
新セッション採用時は、受信ウィンドウとフラグメントバッファをリセットします。
...
通番は送信スレッドが DATA パケットまたは PING パケットを送出する際に付与します。
...
送信スレッドは DATA パケットと PING パケットを送出するたびに送信ウィンドウに登録します。
...
次のパケットを受信したとき:
1. `health_alive` を 1 に設定する
2. `POTR_EVENT_CONNECTED` をコールバックで通知する

### PING パケットの性質

- PING パケットにも通番が付与され、送信ウィンドウに登録されます（再送対象です）
