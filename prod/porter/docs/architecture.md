...
`POTR_TYPE_UNICAST_BIDIR` では、**両端が送受信・NACK・ヘルスチェックをすべて独立して行います**。
役割 (Role) はデータ方向を意味せず、アドレス設定の対称性を識別するラベルとしてのみ機能します。

| Role | 意味 |
|---|---|
| `POTR_ROLE_SENDER` | `dst_addr` / `dst_port` を相手アドレスとして使用する側 |
| `POTR_ROLE_RECEIVER` | `src_addr` / `src_port` を自側として設定する側 |

> UDP は無接続であるため、どちらの端が先に `potrOpenService()` を呼んでも動作に違いはありません。
...
### unicast_bidir のスレッド構成（両端共通）

`POTR_TYPE_UNICAST_BIDIR` では両端が対称なスレッド構成を持ちます。

```plantuml
@startuml unicast_bidir スレッド構成
title unicast_bidir のスレッド構成（両端共通）
...
rectangle "porter ライブラリ (unicast_bidir、両端共通)" {
  queue "送信キュー\n(リングバッファ 1024 要素)" as Q
  [送信スレッド] as ST
  [受信スレッド] as RT
  [ヘルスチェックスレッド] as HT
...
RT --> HT : PING 応答受信通知（last_recv 更新）
...
| スレッド | 役割 |
|---|---|
| 送信スレッド | 送信キューからエレメントを取り出し、DATA パケットを構築して全パスへ `sendto` する。NACK 受信通知に応じて再送する |
| 受信スレッド | DATA / NACK / REJECT / PING 要求 / PING 応答を受信する。PING 要求（`ack_num=0`）には即 PING 応答を返す。DATA はコールバックへ渡す。欠番検出時は NACK を送出する。`check_health_timeout()` でタイムアウトを監視し、超過時に `POTR_EVENT_DISCONNECTED` を発火する |
| ヘルスチェックスレッド | 最終送信時刻を監視し、`health_interval_ms` が経過したら PING 要求（`ack_num=0`）を送信する |
...
> **unicast_bidir の RECEIVER について**: `POTR_TYPE_UNICAST_BIDIR` の `POTR_ROLE_RECEIVER` は、上記に加えて送信ウィンドウ・送信キュー・送信スレッド・ヘルスチェックスレッド・ヘルスチェック用ミューテックス/条件変数・最終送信時刻も保持します。
...
| src_port | 省略可（SENDER）/ 必須（RECEIVER） | 無視 | 無視 | **両端必須** |
| 自動再接続 | なし | ○（SENDER） | ○（SENDER） | なし |
| マルチパス | ○（最大 4 経路） | × | × | ○（最大 4 経路） |
| コールバック | RECEIVER 必須 | RECEIVER 必須（SENDER 任意） | 両端必須 | 両端必須 |
