...
#### UDP 通信種別（`unicast` / `multicast` / `broadcast` / `unicast_bidir` / RAW モード）
...
UDP の `udp_health_timeout_ms` は RECEIVER 側の受信無音タイムアウトですが、
TCP の `tcp_health_timeout_ms` は SENDER 側の PING 応答待機タイムアウトです。
また TCP では DATA 送信頻度に関わらず `tcp_health_interval_ms` 周期で PING を送信します
（UDP は DATA が流れていれば PING を省略）。
...
| `src_addr` | 文字列 | 条件付き | 通常は送信元 bind アドレスまたは受信側の送信元 IP フィルタ。`unicast_bidir` の N:1 サーバでは省略可能 |
...
`unicast_bidir` は `src_addr` / `src_port` の指定有無により、既存の **1:1 双方向通信** と **N:1 サーバ動作** を切り替えます。

| src 情報の指定 | bind 動作 | 送信元フィルタ | ピア数 | 動作モード |
|---|---|---|---|---|
| `src_addr` + `src_port` 指定 | 1:1 として動作。SENDER は `src_addr:src_port`、RECEIVER は `dst_addr:dst_port` で bind | あり (アドレス + ポート) | 1 | 1:1 |
| `src_addr` のみ指定 (`src_port` 省略/0) | 1:1 として動作。SENDER は `src_addr:エフェメラル` で bind | あり (アドレス) | 1 | 1:1 |
| `src_port` のみ指定 (`src_addr` なし) | `dst_addr:dst_port` で bind | あり (ポートのみ) | N | N:1 |
| `src_addr`・`src_port` いずれも未指定 | `dst_addr:dst_port` で bind | なし | N | N:1 |

| キー | 型 | 必須 | 説明 |
|---|---|---|---|
| `src_addr` | 文字列 | 条件付き | 指定した場合は 1:1 モードとして動作する。省略すると N:1 モードとなる |
| `src_port` | uint16 | 省略可 | 1:1 モードでは SENDER の bind ポート。`0` または省略でエフェメラルポートを使用する。N:1 モードでは送信元ポートフィルタとして扱い、`0` または省略でフィルタしない |
| `dst_addr` | 文字列 | 条件付き | 1:1 モードでは相手端アドレス。N:1 モードではサーバの bind アドレス。省略時は `INADDR_ANY` で bind する |
| `dst_port` | uint16 | 必須 | 1:1 モードでは相手端ポート。N:1 モードではサーバの受信ポート |
| `max_peers` | uint32 | 省略可 | N:1 モード時の最大同時接続ピア数。既定値は `1024` |
...
| 暗号化範囲 | DATA パケットのペイロード部分を AES-256-GCM で暗号化する。ヘッダー 32 バイトは平文 |
| AAD | ヘッダー 32 バイトを追加認証データ (AAD) として使用するため、ヘッダー改ざんも検知する |
| 認証タグ (DATA) | 16 バイトの GCM 認証タグを暗号文末尾に付与する。実効ペイロードが `max_payload - 16` バイトに減少する |
| 認証タグ (その他) | PING / NACK / REJECT / FIN は平文ペイロードが 0 バイトだが、AAD (ヘッダー 32B) に対して 16 バイトの GCM 認証タグのみを付与する。ヘッダー改ざんを検知できる |
...
| 送信先アドレス | `dst_addr` | `src_addr` |
| 送信先ポート | `dst_port` | `src_port`（`0` / 省略時は受信した送信元ポートを動的学習） |
| 送信元フィルタ | — | `src_addr`（必要に応じて `src_port` も使用） |

#### N:1 モード

```plantuml
@startuml
title unicast_bidir N:1 のソケット bind / sendto
...
note over CA: 既存の 1:1 unicast_bidir クライアント
note over CB: 既存の 1:1 unicast_bidir クライアント
...
- `unicast` / `multicast` / `broadcast` / `unicast_bidir` 1:1 モード: `src_addr` を用いて送信元 IP アドレスを照合します
- `unicast_bidir` N:1 モード: `src_addr` は照合しません。`src_port` が 0 以外のときのみ送信元ポートを照合します

一致しないパケットはアプリケーション層で破棄します。

### 1:1 モードの制約

1:1 モードの `unicast_bidir` では、1 つのサービス定義が同時に複数ピアを受け付けることはありません。複数クライアントを動的に受け付けたい場合は、`src_addr` を省略した N:1 モードを使用します。
...
; N:1 サーバ: src 情報を省略し、dst_port で待ち受ける
[service.4050]
type      = unicast_bidir
dst_addr  = 0.0.0.0
dst_port  = 9050
