...
typedef enum {
    ...
    POTR_TYPE_TCP_N1            = 10,  /* 追加: TCP 単方向 N:1 */
    POTR_TYPE_TCP_BIDIR_N1      = 11,  /* 追加: TCP 双方向 N:1 */
} PotrType;
```
...
同一 SENDER が path 0 で接続済みの状態で path 1 から追加接続する場合、RECEIVER は別ピアとして扱うべきか、同一ピアの追加パスとして扱うべきかが問題になる。

現在の `peer_create_tcp()` の設計は「path ごとに独立した accept」を想定しており、path 0 の accept で peer_A を作成し、path 1 の accept で peer_A に `tcp_conn_fd[1]` を追加する仕組みになっていない。

**今回の方針**: 各 accept は独立したピアとして扱う (シンプルな実装)。マルチパスを 1 ピアとして束ねる機能は将来課題とする。

SENDER 側でマルチパスを使う場合は、各パスの接続を独立した `POTR_TYPE_TCP` サービスとして openService するか、将来実装するパスバンドリング機能を使うこと。
...
| `prod/porter/libsrc/porter/thread/potrConnectThread.c` | 変更 | `receiver_accept_n1_loop()` 追加; `connect_thread_func()` に型判定分岐 |
