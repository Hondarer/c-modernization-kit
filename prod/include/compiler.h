 ...
 */
    ...
    #define COMPILER_NAME  "name"    /*!< コンパイラ名の文字列です ("MSVC", "Clang", "GCC", "Unknown")。 */
    #define COMPILER_VERSION 0       /*!< コンパイラバージョンの数値です。 */
#else
    #if defined(_MSC_VER)
...
/**
 *  @name           インライン制御マクロ
 *  @brief          コンパイラ固有のインライン制御属性を抽象化します。
 *
 *  @details
 *  コンパイラごとに異なるインライン強制・抑制の構文を、
 *  統一的なマクロで提供します。
 *
