...
C 言語のソースコードを対象として、Linux/Windows 両対応のコーディング、テスト、ドキュメント生成までのすべての作業を一貫して実施できる仕組みの例を、実際の環境に限りなく近い形で提供しています。
...
- Docs as Code: Doxygen と Doxybook を利用した包括的なドキュメント生成システム
- ドキュメント発行: Pandoc を利用した html と docx 形式の出力
- サンプルコード: 実際のプロジェクトでの使用例を想定したサンプルコードにより、イメージがつきやすい
