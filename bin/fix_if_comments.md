...
`#else` はすべての条件が偽であることを示し、`#endif` はコメントなし。
...
**`#endif` にコメントを付けない理由:**  
`#ifdef MACRO` のように単一マクロで閉じるブロックは `/* MACRO */` で識別できるが、
`#if defined(A) / #elif defined(B)` チェーンは複数マクロを一つの識別子で表現できない。
`/* MACRO_* */` のようなワイルドカード記法は非標準であるため、コメントなしが適切。
...
- `#if defined(MACRO)` → スタックに `{kind='if_defined', macros=[MACRO], has_else=False}` を積む
- `#elif defined(MACRO)` → スタックトップが `if_defined` の場合、`macros` リストに追記
...
- `#else` の行:
  ...
  - `#if defined()` 対応 → `/* !MACRO_A && !MACRO_B && ... */`
- `#endif` の行:
  - `#ifdef`/`#ifndef` 対応 → `/* MACRO */`（`#else` の有無を問わず）
  - `#if defined()` 対応 → コメントなし（既存コメントがあれば削除）
