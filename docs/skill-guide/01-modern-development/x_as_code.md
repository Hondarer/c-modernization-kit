...
- **`framework/makefw/`** (サブモジュール) - Make ビルドシステムをコード化したフレームワークです。C/C++ および .NET のビルドルールをテンプレートとして定義し、プロジェクト横断で再利用できます。
- **`framework/testfw/`** (サブモジュール、論理名: `testfw`) - テスト実行・レポート生成をコード化しています。Google Test ベースのテストパイプラインを makefile で制御します。

### Documentation as Code

- **`framework/doxyfw/`** (サブモジュール、論理名: `doxyfw`) - Doxygen・Doxybook2 を使い、ソースコードコメントから HTML/Markdown ドキュメントを自動生成します。ドキュメントをコードと同一リポジトリで管理する Design as Code の実践例です。
- **`framework/docsfw/`** (サブモジュール、論理名: `docsfw`) - Pandoc を使った Markdown → HTML/docx 変換フレームワークです。ドキュメントの発行プロセスもコード化されています。
- **`docs/`** - ADR に相当するドキュメントソース群です。設計上の決定や背景をコードと並列管理しています。
...
本リポジトリの `docs` および各サブモジュールの `docs` は、開発者に加えて AI へのコンテキスト提供も意図したドキュメントです。「コードベースの品質が生成 AI の活用効果を左右する」という発表の主張を体現しています。
