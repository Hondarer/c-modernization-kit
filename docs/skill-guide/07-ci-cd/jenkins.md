...
- warn ZIP がある場合は、`index.html` で通常アーティファクトとは別に「ビルド時の警告内容詳細」として表示される
...
ビルドスクリプトにより `source/pages/index.html` が自動生成されるため、アクセス時に Doxygen API ドキュメント (モジュール別)・言語別ドキュメント・通常アーティファクトへのリンクが表示されます。warn ZIP がある場合は、別セクション「ビルド時の警告内容詳細」に分けて表示されます。
