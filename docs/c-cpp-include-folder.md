...
`app/<name>/` 配下の `makepart.mk` では `$(MYAPP_FOLDER)` を使用します。
自 app 内のパスは `$(MYAPP_FOLDER)/...`、他 app のパスは `$(MYAPP_FOLDER)/../{otherapp}/...` と記述します。
`$(MYAPP_FOLDER)` の詳細は [makeparts.md](../framework/makefw/docs/makeparts.md) を参照してください。

`app/makepart.mk` (app/ 直下) やワークスペースルートの `makepart.mk` では、従来どおり `$(WORKSPACE_FOLDER)` を使用します。
