...
| `TESTFW_HOME` | `${{ github.workspace }}/framework/testfw` | テストフレームワークの場所 |
...
4. **ドキュメント生成**
   ...
   - `make doxy` は `DOXYFW_HOME`、`make docs` は `DOCSFW_HOME` を参照
...
2. **アーティファクトの整理と統合**
   ...
   - Linux OL8 ビルドログを `linux-ol8-logs.zip` にアーカイブ
   - Linux OL10 ビルドログを `linux-ol10-logs.zip` にアーカイブ
   - Windows ビルドログを `windows-logs.zip` にアーカイブ
