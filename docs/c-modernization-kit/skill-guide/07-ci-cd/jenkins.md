...
- `LD_LIBRARY_PATH`: `app/calc/prod/lib`, `app/calc.net/prod/lib`, `app/override-sample/prod/lib`, `app/porter/prod/lib`, `app/com_util/prod/lib`
...
`source/.jenkins/build.sh` から呼び出される `inner-build.sh` は、コンテナ内で `DOCSFW_HOME=/workspace/framework/docsfw` と `DOXYFW_HOME=/workspace/framework/doxyfw` を既定値として設定します。
別の framework 配置を使う場合は、Jenkins ジョブ側でコンテナ内から参照できるパスを環境変数として渡してください。

この構成では、コンテナ内でまず `devcontainer-entrypoint.sh` が実行され、`HOST_USER` / `HOST_UID` / `HOST_GID` に基づいてユーザーとホームディレクトリが初期化されます。その後、作成済みユーザー権限で `make` などを実行します。コンテナ処理の完了後は `report-warnings.sh` が `source/pages/artifacts/*-warns.zip` を検知し、Jenkins の Console Output に通知を出します。
...
- `make` が成功する
- `make test` が成功する
- `BUILD_DOCS="1"` の場合に `DOCSFW_HOME` / `DOXYFW_HOME` が `/workspace/framework/docsfw` / `/workspace/framework/doxyfw` を指している
...
ビルドスクリプトにより `source/pages/index.html` が自動生成されるため、アクセス時に Doxygen API ドキュメント (モジュール別)・言語別ドキュメント・通常アーティファクトへのリンクが表示されます。warn ZIP がある場合は、別セクション「ビルド・ドキュメント警告詳細」に分けて表示されます。ここにはビルド・テスト警告の `linux-ol8-warns.zip` などに加えて、`docs.warn` と Doxygen 警告を束ねた `docs-warns.zip` も表示されます。
