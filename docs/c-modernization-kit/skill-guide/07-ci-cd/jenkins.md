...
`source/.jenkins/build.sh` から呼び出される `inner-build.sh` は、コンテナ内で `DOCSFW_HOME=/workspace/framework/docsfw`、`DOXYFW_HOME=/workspace/framework/doxyfw`、`TESTFW_HOME=/workspace/framework/testfw` を既定値として設定します。
