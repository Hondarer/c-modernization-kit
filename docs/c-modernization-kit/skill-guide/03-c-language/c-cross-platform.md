...
このリポジトリは Linux (GCC) と Windows (MSVC) の両方をサポートするクロスプラットフォーム設計になっています。`app/calc/prod/include/libcalc.h` では、Windows の DLL エクスポート宣言(`__declspec(dllexport)`)と Linux の可視性属性(`__attribute__((visibility("default")))`)を条件コンパイルで切り替えています。同様のパターンは `app/*/prod/` 配下の他のヘッダーファイルにも見られます。

ビルドシステム (makefile) 側でも OS やコンパイラの違いを吸収しており、`framework/makefw/` サブモジュールが Linux / Windows の差異をテンプレートとして提供しています。
...
- [ ] `app/calc/prod/include/libcalc.h` のクロスプラットフォーム宣言を読み取れる
...
クロスプラットフォーム宣言のパターン (`app/calc/prod/include/libcalc.h` の例):
...
- `app/calc/prod/include/libcalc.h` - 動的ライブラリのエクスポート宣言
- `app/calc/prod/include/libcalcbase.h` - 静的ライブラリのヘッダー
- `app/calc/prod/include/libcalc_const.h` - OS に依存しない定数定義
