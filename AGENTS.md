...
- 自動ステージング、コミット禁止。指示があるまでステージング、コミットは行わないこと。
  エージェント作業にあわせてユーザーがステージングする場合はある。アージェントは Git の状態参照のみが許可されており、自動アンステージも禁止する。
...
- `app/` 配下のコードでは `goto` を使用しない。既存コードを変更する際に `goto` があった場合は、`goto` の除去を行う。
- `app/` 配下のコードでは三項演算子を使用しない。既存コードを変更する際に三項演算子があった場合は、三項演算子の除去を行う。
